...
Harness leverages Claude Code's agent team system to decompose complex tasks into coordinated teams of specialized agents. Say "build a harness for this project" and it automatically generates agent definitions (`.trae/agents/`) and skills (`.trae/skills/`) tailored to your domain.
...
Phase 2: Team Architecture Design (Agent Teams vs Subagents)
    ↓
Phase 3: Agent Definition Generation (.trae/agents/)
    ↓
Phase 4: Skill Generation (.trae/skills/)
    ↓
...
```shell
# Copy the skills directory to ~/.trae/skills/harness/
...
- [Agent Teams enabled](https://code.trae.com/docs/en/agent-teams): `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`
